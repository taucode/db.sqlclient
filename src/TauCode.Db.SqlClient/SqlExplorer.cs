...
public class SqlExplorer : Explorer
{
    #region Constants
    ...
    public SqlExplorer(SqlConnection connection)
        : base(connection)
    {
    }
    ...
    protected int GetTableObjectId(string schemaName, string tableName)
    {
        ...
        using var command = (SqlCommand)this.Connection.CreateCommand();
        command.CommandText =
            @"
SELECT
    T.object_id
...
";
    ...
    protected override IReadOnlyList<string> GetTableNamesImpl(string schemaName)
    {
        ...
        var connection = (SqlConnection)this.GetOpenConnection();
        using var command = connection.CreateCommand();
